...
void write_file (const vector<item_info> itemlist, const int items_per_file, const string dest_dir, const string prefix) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    cout << hex << magic_number << flag_and_length << endl;
...
int main (int argcnt, char **argval) {
  if (argcnt <= 5) {
    ...
    cout << "warning : only support 4-bit label and 4-bit feats data. which means label data type hould be uint32_t             " << endl;
